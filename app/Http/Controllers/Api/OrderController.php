   ...
   public function store(Request $request)
{
    ...
    try {
        ...
        // Create inventory log and reduce stock
        InventoryLog::create([
            'drug_id' => $request->drug_id,
            'user_id' => Auth::id(),
            'change_type' => 'sale',
...
public function updatePrescriptionStatus(Request $request, $orderId)
{
    ...
    public function getPharmacistOrders(Request $request)
    {
        ...
        try {
            ...
            $revenueByCategory = $orders->filter(function ($order) {
                return $order->status === 'paid';
            })->groupBy('drug.category')
            ->map(function ($categoryOrders) {
                return $categoryOrders->sum(function ($order) {
                    return $order->drug->price * $order->quantity;
                });
            });
    
            // Calculate revenue by prescription needed
            $revenueByPrescription = $orders->filter(function ($order) {
                return $order->status === 'paid';
            })->groupBy('drug.prescription_needed')
            ->map(function ($prescriptionOrders) {
                return $prescriptionOrders->sum(function ($order) {
                    return $order->drug->price * $order->quantity;
                });
            });
    
            // Calculate revenue by month
            $revenueByMonth = $orders->filter(function ($order) {
                return $order->status === 'paid';
            ...
            return response()->json([
                ...
                'meta' => [
                    ...
                    'revenue_by_prescription' => $revenueByPrescription,
